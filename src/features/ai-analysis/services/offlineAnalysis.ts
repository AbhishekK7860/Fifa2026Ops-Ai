...
import type { AnalysisResponse, ConfidenceBound } from "@/types/analysis";
...
function buildStatusTemplate(row: GateRow): StatusTemplate {
  ...
  switch (row.status) {
    case "critical":
      return {
        observation:
          `${row.gate} is at critical density: ${row.currentVisitors.toLocaleString()} visitors ` +
        ...
        reasoning:
          `Critical status with ${occupancyPct}% occupancy and a queue-to-volunteer ratio of ${qvRatio} ` +
        ...
        action:
          "Immediately activate overflow protocol: redirect incoming visitors to nearest " +
        ...
        impact:
          "Halting entry and activating overflow should reduce queue-to-volunteer ratio within 10–15 minutes " +
          "and prevent escalation to a safety incident.",
        en:
          `Attention: ${row.gate} is currently at critical capacity. Please follow volunteer directions ` +
          "and proceed to an alternate gate if indicated.",
        es:
          `Atención: ${row.gate} está actualmente a capacidad crítica. Por favor siga las instrucciones ` +
          "del voluntario y diríjase a una puerta alternativa si se indica.",
        fr:
          `Attention : ${row.gate} est actuellement à capacité critique. Veuillez suivre les instructions ` +
    ...
    case "busy":
      return {
        observation:
          `${row.gate} is operating at elevated occupancy: ${row.currentVisitors.toLocaleString()} visitors ` +
        ...
        reasoning:
          `Busy status at ${occupancyPct}% occupancy warrants close monitoring. ` +
        ...
        action:
          "Increase volunteer visibility at the gate entrance. Monitor queue length every 5 minutes. " +
          "Prepare overflow redirect messaging and request one standby volunteer.",
        impact:
          "Early monitoring and standby staffing typically prevent escalation to critical status " +
          "and reduce wait times by 20–30%.",
        en:
          `${row.gate} is currently busy. Please have your ticket ready and follow ` +
          "volunteer instructions to keep the queue moving smoothly.",
        es:
          `${row.gate} está actualmente ocupada. Por favor tenga su boleto listo y siga ` +
          "las instrucciones del voluntario para mantener la fila en movimiento.",
        fr:
          `${row.gate} est actuellement occupée. Veuillez avoir votre billet prêt et suivre ` +
    ...
    default: // normal
      return {
        observation:
          `${row.gate} is operating normally: ${row.currentVisitors.toLocaleString()} visitors ` +
          `(${occupancyPct}% of capacity), queue length ${row.queueLength.toLocaleString()}, ` +
          `${row.volunteerCount} volunteer${row.volunteerCount !== 1 ? "s" : ""} on duty.`,
        reasoning:
          `Normal status at ${occupancyPct}% occupancy. Queue-to-volunteer ratio of ${qvRatio} is within acceptable range. ` +
        ...
        action:
          "Maintain standard volunteer presence. Continue routine monitoring on a 15-minute interval. " +
          "No escalation actions needed at this time.",
        impact:
          "Continued routine monitoring maintains normal operations and ensures timely detection " +
          "of any developing issues.",
        en:
          `${row.gate} is open and operating normally. Welcome — please proceed to your seat.`,
        es:
          `${row.gate} está abierta y funcionando con normalidad. Bienvenido, por favor diríjase a su asiento.`,
        fr:
          `${row.gate} est ouverte et fonctionne normalement. Bienvenue — veuillez vous diriger vers votre siège.`,
      };
  }
}
...
function buildSourceRefs(row: GateRow): string[] {
  return [
    `Gate: ${row.gate}`,
    `Status: ${row.status}`,
...
export function runOfflineAnalysis(
  ...
  return {
    ...
    confidence: {
      score: bound.value,
      basis:
        `Offline analysis — local confidence bound of ${bound.value} ` +
